...
def calcola_media_emozioni(file_excel, output_csv):
    wb = openpyxl.load_workbook(file_excel)
    foglio = wb.worksheets[0]
    etichette = ['Angry', 'Happy', 'Sad', 'Neutral']
    nomi_video = []
    emozioni = {etichetta: [] for etichetta in etichette}
    print("EMOZIONI:\n")
    print(emozioni)

    # Estrarre i nomi dei video
    for cella in foglio[1][1:]:
        if isinstance(cella.value, str):
            nomi_video.append(cella.value)

    # Calcolare le medie per ciascuna emozione
    for i, etichetta in enumerate(etichette, start=2):  # Supponendo che le emozioni inizino dalla colonna 2
        for colonna in foglio.iter_cols(min_row=3, min_col=i):
            valori_colonna = [cella.value for cella in colonna if isinstance(cella.value, (int, float))]
            media = sum(valori_colonna) / len(valori_colonna) if valori_colonna else 0
            emozioni[etichetta].append(media)
    
    print("EMOZIONI:\n")
    print(emozioni)

    print("LEN:\n")
    print(emozioni['Angry'].len())
    # Scrivere il CSV con le medie per ogni emozione
    with open(output_csv, mode='w', newline='') as file_csv:
        writer = csv.writer(file_csv)
        # Intestazione
        writer.writerow(['Nome Video'] + etichette)
        # Riga per ciascun video
        for i, nome_video in enumerate(nomi_video):
            print(emozioni[etichetta][i])
            riga = [nome_video] + [emozioni[etichetta][i] for etichetta in etichette]
...
def main():
    ...
    risultato = calcola_media_emozioni(file_excel)
